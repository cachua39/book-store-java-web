...
public class PaypalServices implements Serializable {
    ...
    private static final String URL_RETURN = "http://localhost:8084/J3.L.P0002/executePayment";
    private static final String URL_CANCEL = "http://localhost:8084/J3.L.P0002/customer_cart.jsp";
    ...
    private List<Transaction> getTransactions(OrderDTO orderDTO) {
        ...
        for(BookDTO book : orderDTO.getListBook()){
            ...
            item.setPrice(String.format("%.2f", book.getPrice()));
            item.setQuantity(String.valueOf(book.getQuantityCart()));
            item.setTax("0.0");
            items.add(item);
        }
        ...
        if(orderDTO.getCouponCode() != null && !orderDTO.getCouponCode().isEmpty()) {
           ...
           item.setPrice(String.format("%.2f", (0-orderDTO.getDiscountValue())));
           item.setTax("0.0");
           item.setQuantity("1");
           items.add(item);
        }
